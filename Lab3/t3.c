...
void *producer(void *arg)
{
    ...
    while (!producer_done)
    {
        ...
        while(supply > 0)//you have to wait until it hits 0
            pthread_cond_wait(&producer_cv, &mut);

        if(num_cons_remaining == 0){//if there are no more consumers then you are done
            producer_done = 1;
            pthread_mutex_unlock(&mut);
            break;
        }

        printf("Producer thread produced 10 items\n");//otherwise go ahead and start
        fflush(stdin);//producing more items

        supply +=10;//add them

        if(supply>0)//then call the consumer to work
